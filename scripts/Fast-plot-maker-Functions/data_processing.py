...
Handles measurement data loading, processing, and summarization
"""
...
def create_summary_files_for_pipeline(measurements_folder):
    """
    Modified version of create_summary_files that takes a folder path parameter
    """
...
def load_summarized_data(base_dir):
    """Load the summarized data files created by R scripts"""
    print("\n" + "="*50)
    print("LOADING SUMMARIZED DATA")
    print("="*50)
    
    # Look for summarized files in the summarized directory
    summarized_dir = os.path.join(base_dir, "summarized")
    
    if not os.path.exists(summarized_dir):
        print(f"Warning: Summarized directory not found: {summarized_dir}")
        return {}
    
    # Load all CSV files in the summarized directory
    summarized_data = {}
    csv_files = [f for f in os.listdir(summarized_dir) if f.endswith('.csv')]
    
    for filename in csv_files:
        file_path = os.path.join(summarized_dir, filename)
        try:
            df = pd.read_csv(file_path)
            condition = filename.replace('.csv', '').replace('_summary', '')
            summarized_data[condition] = df
            print(f"Loaded {condition}: {df.shape[0]} rows, {df.shape[1]} columns")
        except Exception as e:
