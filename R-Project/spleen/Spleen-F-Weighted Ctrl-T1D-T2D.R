...
p <- ggplot(weighted_summary_data, aes(x = factor(group, levels = c("Control", "T1D", "AAb")), y = weighted_percentage, fill = group)) +
  ...
  scale_x_discrete(labels = c("Control" = "ND", "T1D" = "T1D", "AAb" = "AAb")) +
  labs(
    x = NULL,
...
cat("\nPairwise one-sided tests (greater) vs Control for disease groups (T1D, AAb).\n")
