    ...
    if EXCEL_ENGINE:
        ...
        with pd.ExcelWriter(excel_path, engine=EXCEL_ENGINE) as writer:
            ...
            for group in groups:
                ...
                print(f"Averaging {num_files} files for {sheet_name} group...")
                ...
                column_headers = ['Count', f'{sheet_name}_avg']
                ...
                print(f"Created sheet '{sheet_name}' with averaged data from {num_files} files")
                
                # Print some statistics for verification
                print(f"  Sample averages for {sheet_name}:")
                for i, row_label in enumerate(['Num Cells', 'Num 1+', 'Prop_1+']):
                    if i < len(data_matrix):
                        print(f"    {row_label}: {data_matrix[i][1]}")
            
            # Create Summary sheet with averages across all conditions
            if sheets_created > 0:
                # Calculate averages for each classification across all groups
                summary_data = []
                ...
                print(f"Created 'Summary' sheet with averaged values across all conditions")
