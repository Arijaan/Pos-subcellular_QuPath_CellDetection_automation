...
def main():
    """
    Main workflow function - supports both single file and batch processing
    """
    print("QuPath Measurement Analyzer - Complete Workflow")
    print("=" * 60)
    ...
    print("2. Process all existing summary Excel files in a directory")
    ...
    if mode == "1":
        ...
        if not scripts_to_run:
            print("No R scripts found. Analysis complete with Combined Summary only.")
            cleanup_tkinter()
            return
        ...
        for analysis_name, script_path in scripts_to_run:
            ...
            success = run_r_script(script_path, combined_summary_path, output_base, analysis_name)
        ...
        print("=" * 60)
        print(f"📁 Main output directory: {measurements_folder}")
        print(f"📊 Combined Summary: {combined_summary_path}")
        
        # List generated files
        measurement_summary_dir = os.path.join(measurements_folder, "Measurement_summary")
        if os.path.exists(measurement_summary_dir):
            print(f"📈 Additional files in: {measurement_summary_dir}")
            files = os.listdir(measurement_summary_dir)
            for file in files:
                print(f"   - {file}")
        ...
        # New workflow: Process existing summary files
        print("\nSelect directory containing summary Excel files...")
        if hasattr(get_user_groups, 'root') and get_user_groups.root:
            directory_path = filedialog.askdirectory(
                parent=get_user_groups.root,
                title="Select directory containing summary Excel files"
            )
        else:
            root = tk.Tk()
            root.withdraw()
            directory_path = filedialog.askdirectory(title="Select directory containing summary Excel files")
            root.destroy()
        ...
        results = process_all_summary_files(directory_path, groups)
        ...
        print("\n📈 Results locations:")
        for file_path, success, output_dir in results:
            status = "✅" if success else "❌"
            rel_path = os.path.relpath(file_path, directory_path)
            print(f"   {status} {rel_path}")
            print(f"      → {os.path.relpath(output_dir, directory_path)}")
